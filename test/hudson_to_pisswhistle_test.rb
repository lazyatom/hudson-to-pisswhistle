...
context "Given a build.xml file in a workspace" do
  ...
  setup do
    ...
    @build_xml = setup_build_xml_for(project_name, build_xml_string, changelog)
  ...
  context "when running the script" do
    ...
    context "the posted data" do
      ...
      should "include a message" do
        assert_equal "test-project (def) built successfully", @net_lib.payload["message"]
      end

      should "include a top-level success for compatibility" do
